 ...
 *
 * dynamic_cast/visitable_as.hxx --
 *   Provides the helper class to implement the dynamic dispatch for the accept
 *   method in a visitable hierarchy.
 *   Uses CRTP to provide the required accept() member function to classes by
 *   plugging itself into the hierarchy.
 */

#ifndef DYN_VISITABLE_AS_HXX
#define DYN_VISITABLE_AS_HXX
...
struct visitable_as : Base {
     ...
     * If it is of the correct type, we call visit on it.
     *
     * \param dyn_vtor The dynamic visitor
     */
    void accept(visitor_base& dyn_vtor) override {
        // we try to downcast it into the type *we* need to visit us
        if (auto vtor = dynamic_cast<visitor_impl<T>*>(&dyn_vtor)) {
            auto self = static_cast<T*>(this);
            assert(self);

            // and let it visit us, if it's correct (not nullptr)
            vtor->do_visit(self);
        }
    }
